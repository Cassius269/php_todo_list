...
if($_SERVER["REQUEST_METHOD"]==="POST"){
    if(isset($_POST["task"]) && !empty($_POST["task"])){
        ...
        $taskAdded->setState("true"); // false : tâche pas encore faite
...
if(isset($_GET["data-id"])){
    ...
    $taskRepository->updateState($_GET["data-id"]);
    ...
    <main>
        ...
        <ul id="liste">
            <?php foreach($tasks as $task):?>
                <li>
                <a href=<?= "?data-id=" . $task->getiD(); ?>><span class=<?= $task->getState() === false ? null : "finished" ?>><?= $task;?></span></a>
